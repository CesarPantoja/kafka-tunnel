...
def aws(jump_host, zookeeper_port, kafka_port, region, profile):
    instances = []
    click.echo(
        ' * retrieving ip\'s from AWS ({},{}) zookeeper/kafka ec2 instances by tag_name ...'.format(profile, region))
...
def remove_local_interfaces(instances):
    click.echo(' * removing interfaces, user/root password might be needed')
    for instance in instances:
        if sys.platform == 'darwin':
            cmd = ['sudo', 'ifconfig', 'lo0', '-alias', instance.ip]
        else:
            cmd = ['sudo', 'ip', 'a', 'del', instance.ip+"/32", 'dev', 'lo']
        subprocess.call(cmd)
